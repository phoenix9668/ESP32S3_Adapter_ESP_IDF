...
static bool parse_integer(const std::string& text, int& value) {
    ...
    if (errno != 0 || end != text.c_str() + text.size() || parsed < INT_MIN || parsed > INT_MAX) {
        return false;
    }
...
bool AtUart::ParseResponse() {
    ...
    if (!command.empty()) {
        ...
        while (field_start <= values.size()) {
            ...
            argument.raw_value = item;
            argument.int_value = 0;
            argument.double_value = 0.0;
            if (item.size() >= 2 && item.front() == '"' && item.back() == '"') {
                argument.type = AtArgumentValue::Type::String;
                argument.string_value = item.substr(1, item.size() - 2);
            } else if (parse_integer(item, argument.int_value)) {
                argument.type = AtArgumentValue::Type::Int;
                argument.string_value = item;
                argument.double_value = static_cast<double>(argument.int_value);
            } else if (parse_double(item, argument.double_value)) {
