  ...
  while (1) {
    ...
    if (rxBytes > 4) {
      ...
      if (ch9434_init_end) {
        ...
        if (data[rxBytes - 4] == ((crc32_result >> 24) & 0xFF) &&
            data[rxBytes - 3] == ((crc32_result >> 16) & 0xFF) &&
            data[rxBytes - 2] == ((crc32_result >> 8) & 0xFF) &&
            data[rxBytes - 1] == (crc32_result & 0xFF)) {
          if ((data[0] == ((PACKET_HEADER >> 8) & 0xFF)) &&
              (data[1] == (PACKET_HEADER & 0xFF)) && (data[2] == 0x00) &&
              (data[3] == (board_address & 0xFF)) && (data[4] == 0x00) &&
              (data[5] == 0x02)) {
        ...
        case 0x06: // receive line status
        {
          ...
          if (rec_buf_cnt) {
            ...
            if (uart_idx == CH9434_UART_IDX_1 && rec_buf_cnt >= 8) {
              ...
              if (xSemaphoreTake(mutex, portMAX_DELAY)) {
                e34_2g4d20d_sendData(CH9434_TASK_TAG, (char *)packet_to_android,
                                     rec_buf_cnt + 10);
                xSemaphoreGive(mutex);
              }
            }
