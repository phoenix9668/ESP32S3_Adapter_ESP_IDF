...
#define UART2_RXD_PIN (GPIO_NUM_1)
...
static void e34_2g4d20d_tx_task(void *arg) {
  ...
  while (1) {
    ...
    if (xQueueReceive(tableQueue, &table_data, portMAX_DELAY) == pdPASS) {
      table_data.data[table_data.length] = 0;
      ESP_LOGD(E34_2G4D20D_TX_TASK_TAG, "Read from queue %d bytes: '%s'",
      ...
      if (table_data.data[table_data.length - 1] == 0x0d ||
        ...
        if (xSemaphoreTake(uart1_mutex, portMAX_DELAY)) {
          e34_2g4d20d_sendData(E34_2G4D20D_TX_TASK_TAG,
                               (char *)packet_to_android,
                               table_data.length + 10);
        ...
        else if (table_data.length >= 12 &&
                 (table_data.data[2] == 0x2b || table_data.data[2] == 0x2d)) {
        ...
        if (modbus_packet_len > 0 && modbus_packet != NULL) {
          ...
          uint16_t calculated_crc =
              calculateModbusCRC16(modbus_packet, modbus_packet_len - 2);
          // Modbus RTU的CRC是低字节在前，高字节在后
          uint16_t received_crc = (modbus_packet[modbus_packet_len - 1] << 8) |
                                  modbus_packet[modbus_packet_len - 2];

          if (calculated_crc == received_crc) {
            // CRC校验通过，使用UART1发送，需要uart1_mutex保护
            if (xSemaphoreTake(uart1_mutex, portMAX_DELAY)) {
              e34_2g4d20d_sendData(E34_2G4D20D_TX_TASK_TAG,
                                   (char *)modbus_packet, modbus_packet_len);
              xSemaphoreGive(uart1_mutex);
            }
            ESP_LOGI(E34_2G4D20D_TX_TASK_TAG, "Sent Modbus RTU data: %d bytes",
                     modbus_packet_len);
            ESP_LOG_BUFFER_HEXDUMP(E34_2G4D20D_TX_TASK_TAG, modbus_packet,
                                   modbus_packet_len, ESP_LOG_INFO);
          } else {
            ESP_LOGW(E34_2G4D20D_TX_TASK_TAG,
                     "Modbus CRC error: calculated=0x%04X, received=0x%04X",
                     calculated_crc, received_crc);
            ESP_LOG_BUFFER_HEXDUMP(E34_2G4D20D_TX_TASK_TAG, modbus_packet,
                                   modbus_packet_len, ESP_LOG_WARN);
          }
        } else {
          ESP_LOGW(E34_2G4D20D_TX_TASK_TAG,
                   "Unknown Modbus format, byte[2]=0x%02X, length=%d",
                   table_data.data[2], table_data.length);
          ESP_LOG_BUFFER_HEXDUMP(E34_2G4D20D_TX_TASK_TAG, table_data.data,
                                 table_data.length, ESP_LOG_WARN);
        }
      }
    }
...
static void e34_2g4d20d_rx_task(void *arg) {
  ...
  // 粘包处理缓冲区
  static uint8_t packet_buffer[RX_BUF_SIZE * 2];
  ...
  while (1) {
    ...
    if (rxBytes > 4) {
      // 将新接收的数据追加到缓冲区
      if (buffer_pos + rxBytes < sizeof(packet_buffer)) {
        memcpy(&packet_buffer[buffer_pos], data, rxBytes);
        buffer_pos += rxBytes;
        ESP_LOGD(E34_2G4D20D_RX_TASK_TAG, "Received %d bytes, buffer total: %d",
                 rxBytes, buffer_pos);
        ESP_LOG_BUFFER_HEXDUMP(E34_2G4D20D_RX_TASK_TAG, data, rxBytes,
                               ESP_LOG_DEBUG);
      } else {
        // 缓冲区溢出，清空重新开始
        ESP_LOGW(E34_2G4D20D_RX_TASK_TAG, "Buffer overflow, reset");
        buffer_pos = 0;
        continue;
      }

      // 处理缓冲区中的完整数据包
      while (buffer_pos >= 10) // 最小包长度：6字节帧头 + 4字节CRC
      {
        // 查找包头 0xE5 0x5E
        int packet_start = -1;
        for (int i = 0; i <= buffer_pos - 10; i++) {
          if (packet_buffer[i] == 0xE5 && packet_buffer[i + 1] == 0x5E) {
            packet_start = i;
            break;
          }
        }
        ...
        if (packet_start == -1) {
          ...
          ESP_LOGD(E34_2G4D20D_RX_TASK_TAG,
                   "No packet header found, clearing buffer");
          buffer_pos = 0;
          break;
        }
        ...
        if (packet_start > 0) {
          // 丢弃包头前的无效数据
          ESP_LOGD(E34_2G4D20D_RX_TASK_TAG,
                   "Discarding %d bytes before packet header", packet_start);
          memmove(packet_buffer, &packet_buffer[packet_start],
        ...
        if (buffer_pos >= 10 && ch9434_init_end) {
          ...
          int packet_len = (next_header > 0) ? next_header : buffer_pos;

          // 确保至少有最小包长度
          if (packet_len >= 10) {
            // 验证CRC（数据长度 = 包长度 - 4字节CRC）
            int data_len = packet_len - 4;
            crc32_result = calculateCRC32(packet_buffer, data_len);

            if (packet_buffer[data_len] == ((crc32_result >> 24) & 0xFF) &&
                packet_buffer[data_len + 1] == ((crc32_result >> 16) & 0xFF) &&
              ...
              if (packet_buffer[0] == 0xE5 && packet_buffer[1] == 0x5E &&
                  ...
                  packet_buffer[3] == (board_address & 0xFF) &&
                ...
                if (packet_buffer[5] == 0x02 && payload_len > 0) {
                  // RFID数据包
                  ESP_LOGD(E34_2G4D20D_RX_TASK_TAG,
                           "Valid RFID packet (%d bytes)", packet_len);
                  ...
                  // 移除已处理的数据包
                  memmove(packet_buffer, &packet_buffer[packet_len],
                  ...
                  // 称重数据包
                  ESP_LOGI(E34_2G4D20D_RX_TASK_TAG,
                           "Valid weight packet (%d bytes)", packet_len);
                  ESP_LOG_BUFFER_HEX(E34_2G4D20D_RX_TASK_TAG, packet_buffer,
                                     packet_len);
          ...
          if (!packet_processed) {
            if (next_header > 0) {
              // CRC校验失败，丢弃到下一个包头之前的数据
              ESP_LOGW(E34_2G4D20D_RX_TASK_TAG,
                       "Invalid packet, skipping to next header");
              memmove(packet_buffer, &packet_buffer[next_header],
                      buffer_pos - next_header);
              buffer_pos -= next_header;
            } else {
              // 没有找到下一个包头，等待更多数据
              break;
        ...
        for (uart_idx = 0; uart_idx < 4; uart_idx++) {
          ...
          case 0x06: // receive line status
          {
            ...
            if (rec_buf_cnt) {
              ...
              if (rec_buf_cnt >= 6) {
                ...
                crc32_result =
                    calculateCRC32(packet_to_android, rec_buf_cnt + 6);
                // 计算并追加CRC校验码
                packet_to_android[rec_buf_cnt + 6] =
                    (crc32_result >> 24) & 0xFF;
                packet_to_android[rec_buf_cnt + 7] =
                    (crc32_result >> 16) & 0xFF;
                ...
                if (xSemaphoreTake(uart1_mutex, portMAX_DELAY)) {
                  e34_2g4d20d_sendData(CH9434_TASK_TAG,
                                       (char *)packet_to_android,
                                       rec_buf_cnt + 10);
                  xSemaphoreGive(uart1_mutex);
                }
          ...
          case 0x0C: // receive data timeout
          {
            ...
            if (rec_buf_cnt) {
              ...
              uart_lsr = CH9434UARTxReadLSR(uart_idx);
              ESP_LOGD(CH9434_TASK_TAG,
                       "idx:%d timeout with empty FIFO, lsr:%02x", uart_idx,
                       uart_lsr);
            }
            break;
          }
...
void app_main(void) {
  ...
  uart2_init();
  ch9434_spi2_init();
  xTaskCreate(rx_task, "uart_rx_task", 1024 * 8, NULL, configMAX_PRIORITIES - 1,
              NULL);
  xTaskCreate(tx_task, "uart_tx_task", 1024 * 8, NULL, configMAX_PRIORITIES - 4,
              NULL);
  xTaskCreate(e34_2g4d20d_rx_task, "e34_2g4d20d_rx_task", 1024 * 8, NULL,
              configMAX_PRIORITIES - 1, NULL);
  xTaskCreate(e34_2g4d20d_tx_task, "e34_2g4d20d_tx_task", 1024 * 8, NULL,
              configMAX_PRIORITIES - 2, NULL);
  xTaskCreate(e34_2g4d20d_reset_task, "e34_2g4d20d_reset_task", 1024 * 8, NULL,
              configMAX_PRIORITIES - 1, NULL);
  xTaskCreate(ch9434_task, "ch9434_task", 1024 * 8, NULL,
