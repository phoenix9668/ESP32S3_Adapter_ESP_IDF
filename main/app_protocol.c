...
bool app_protocol_build_packet(uint8_t address, app_frame_type_t type,
                               ...
                               size_t *out_len) {
  if (payload_len > APP_PAYLOAD_MAX_LEN || out == NULL || out_len == NULL ||
...
bool app_protocol_parse_packet(uint8_t address, const uint8_t *data,
  ...
  if (data == NULL || packet == NULL || length < APP_PACKET_OVERHEAD ||
      length > APP_PACKET_MAX_LEN) {
    return false;
  }

  if (data[0] != ((APP_PACKET_HEADER >> 8) & 0xFFU) ||
      data[1] != (APP_PACKET_HEADER & 0xFFU) || data[2] != 0x00 ||
      data[3] != address || data[4] != 0x00) {
    return false;
  }

  if (data[5] != APP_FRAME_TYPE_WEIGHT && data[5] != APP_FRAME_TYPE_RFID) {
    return false;
  }
  ...
  const uint32_t actual_crc = ((uint32_t)data[crc_pos] << 24) |
                              ((uint32_t)data[crc_pos + 1] << 16) |
                              ((uint32_t)data[crc_pos + 2] << 8) |
                              data[crc_pos + 3];
