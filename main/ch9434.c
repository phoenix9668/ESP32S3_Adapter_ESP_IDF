/********************************** (C) COPYRIGHT
 ******************************** File Name          : CH9434.c Author : tech18
 * Version            : V1.0
 * Date               : 2020/05/08
...
static const char *CH9434_TAG = "CH9434";
...
void CH9434_US_DELAY(void) { esp_rom_delay_us(1); }
 ...
 * Return         : None
 */
void CH9434OscXtFreqSet(uint32_t x_freq) { osc_xt_frequency = x_freq; }
...
void CH9434InitClkMode(uint8_t xt_en, uint8_t freq_mul_en, uint8_t div_num) {
  ...
  spi_read_write_byte(CH9434_REG_OP_WRITE | CH9434_CLK_CTRL_CFG_ADD,
                      clk_ctrl_reg);
  for (i = 0; i < 50000; i++)
    CH9434_US_DELAY();
  ESP_LOGI(
      CH9434_TAG, "CLK_CTRL = %02X",
      spi_read_write_byte(CH9434_REG_OP_READ | CH9434_CLK_CTRL_CFG_ADD, 0xFF));
}
...
void CH9434UARTxParaSet(uint8_t uart_idx, uint32_t bps, uint8_t data_bits,
  ...
  uart_reg_lcr = spi_read_write_byte(
      CH9434_REG_OP_READ | (CH9434_UARTx_LCR_ADD + 0x10 * uart_idx), 0xFF);

  ESP_LOGI(CH9434_TAG, "LCR 1 = %02X", uart_reg_lcr);
  ...
  ESP_LOGI(CH9434_TAG, "LCR 2 = %02X", uart_reg_lcr);
  ...
  ESP_LOGI(CH9434_TAG, "LCR 3 = %02X", uart_reg_lcr);

  spi_read_write_byte(CH9434_REG_OP_WRITE |
                          (CH9434_UARTx_LCR_ADD + 0x10 * uart_idx),
                      uart_reg_lcr);
  ESP_LOGI(CH9434_TAG, "LCR_ADD 1 = %02X",
           spi_read_write_byte(CH9434_REG_OP_READ |
                                   (CH9434_UARTx_LCR_ADD + 0x10 * uart_idx),
                               0xFF));

  spi_read_write_byte(CH9434_REG_OP_WRITE |
                          (CH9434_UARTx_DLL_ADD + 0x10 * uart_idx),
                      uart_reg_dll);
  ESP_LOGI(CH9434_TAG, "DLL_ADD = %02X",
           spi_read_write_byte(CH9434_REG_OP_READ |
                                   (CH9434_UARTx_DLL_ADD + 0x10 * uart_idx),
                               0xFF));

  spi_read_write_byte(CH9434_REG_OP_WRITE |
                          (CH9434_UARTx_DLM_ADD + 0x10 * uart_idx),
                      uart_reg_dlm);
  ESP_LOGI(CH9434_TAG, "DLM_ADD = %02X",
           spi_read_write_byte(CH9434_REG_OP_READ |
                                   (CH9434_UARTx_DLM_ADD + 0x10 * uart_idx),
                               0xFF));
  ...
  spi_read_write_byte(CH9434_REG_OP_WRITE |
                          (CH9434_UARTx_LCR_ADD + 0x10 * uart_idx),
                      uart_reg_lcr);
  ESP_LOGI(CH9434_TAG, "LCR_ADD 2 = %02X",
           spi_read_write_byte(CH9434_REG_OP_READ |
                                   (CH9434_UARTx_LCR_ADD + 0x10 * uart_idx),
                               0xFF));
}
...
void CH9434UARTxFIFOSet(uint8_t uart_idx, uint8_t fifo_en, uint8_t fifo_level) {
  ...
  ESP_LOGI(CH9434_TAG, "uart_reg_fcr = %02X", uart_reg_fcr);

  spi_read_write_byte(CH9434_REG_OP_WRITE |
                          (CH9434_UARTx_FCR_ADD + 0x10 * uart_idx),
                      uart_reg_fcr);
  CH9434_US_DELAY();
  ESP_LOGI(CH9434_TAG, "FCR_ADD = %02X",
           spi_read_write_byte(CH9434_REG_OP_READ |
                                   (CH9434_UARTx_FCR_ADD + 0x10 * uart_idx),
                               0xFF));
}
...
void CH9434UARTxIrqSet(uint8_t uart_idx, uint8_t modem, uint8_t line,
  ...
  spi_read_write_byte(CH9434_REG_OP_WRITE |
                          (CH9434_UARTx_IER_ADD + 0x10 * uart_idx),
                      uart_reg_ier);
  ESP_LOGI(CH9434_TAG, "IER_ADD = %02X",
           spi_read_write_byte(CH9434_REG_OP_READ |
                                   (CH9434_UARTx_IER_ADD + 0x10 * uart_idx),
                               0xFF));
}
...
void CH9434UARTxFlowSet(uint8_t uart_idx, uint8_t flow_en) {
  ...
  uart_reg_mcr = spi_read_write_byte(
      CH9434_REG_OP_READ | (CH9434_UARTx_MCR_ADD + 0x10 * uart_idx), 0xFF);

  ESP_LOGI(CH9434_TAG, "uart_reg_mcr 1 = %02X", uart_reg_mcr);

  uart_reg_mcr &= ~(1 << 5);
  if (flow_en)
    uart_reg_mcr |= (1 << 5);

  ESP_LOGI(CH9434_TAG, "uart_reg_mcr 2 = %02X", uart_reg_mcr);

  spi_read_write_byte(CH9434_REG_OP_WRITE |
                          (CH9434_UARTx_MCR_ADD + 0x10 * uart_idx),
                      uart_reg_mcr);
  ESP_LOGI(CH9434_TAG, "MCR_ADD = %02X",
           spi_read_write_byte(CH9434_REG_OP_READ |
                                   (CH9434_UARTx_MCR_ADD + 0x10 * uart_idx),
                               0xFF));
}
...
void CH9434UARTxWriteSRC(uint8_t uart_idx, uint8_t src_val) {
  spi_read_write_byte(
      CH9434_REG_OP_WRITE | (CH9434_UARTx_SCR_ADD + 0x10 * uart_idx), src_val);
}
...
uint8_t CH9434UARTxReadSRC(uint8_t uart_idx) {
  ...
  uart_reg_src = spi_read_write_byte(
      CH9434_REG_OP_READ | (CH9434_UARTx_SCR_ADD + 0x10 * uart_idx), 0xFF);
...
uint8_t CH9434UARTxReadIIR(uint8_t uart_idx) {
  ...
  uart_reg_iir = spi_read_write_byte(
      CH9434_REG_OP_READ | (CH9434_UARTx_IIR_ADD + 0x10 * uart_idx), 0xFF);
...
uint8_t CH9434UARTxReadLSR(uint8_t uart_idx) {
  ...
  uart_reg_lsr = spi_read_write_byte(
      CH9434_REG_OP_READ | (CH9434_UARTx_LSR_ADD + 0x10 * uart_idx), 0xFF);
...
uint8_t CH9434UARTxReadMSR(uint8_t uart_idx) {
  ...
  uart_reg_msr = spi_read_write_byte(
      CH9434_REG_OP_READ | (CH9434_UARTx_MSR_ADD + 0x10 * uart_idx), 0xFF);
...
uint16_t CH9434UARTxGetRxFIFOLen(uint8_t uart_idx) {
  ...
  spi_read_write_byte(CH9434_REG_OP_WRITE | CH9434_FIFO_CTRL_ADD,
                      uart_fifo_ctrl);

  uart_fifo_cnt_l =
      spi_read_write_byte(CH9434_REG_OP_READ | CH9434_FIFO_CTRL_L_ADD, 0xFF);

  uart_fifo_cnt_h =
      spi_read_write_byte(CH9434_REG_OP_READ | CH9434_FIFO_CTRL_H_ADD, 0xFF);
...
void CH9434UARTxTnowSet(uint8_t uart_idx, uint8_t tnow_en, uint8_t polar) {
  ...
  tnow_ctrl_reg =
      spi_read_write_byte(CH9434_REG_OP_READ | CH9434_TNOW_CTRL_CFG_ADD, 0xFF);
  ...
  spi_read_write_byte(CH9434_REG_OP_WRITE | CH9434_TNOW_CTRL_CFG_ADD,
                      tnow_ctrl_reg);
}
...
void CH9434LowerPowerModeSet(uint8_t mode) {
  ...
  spi_read_write_byte(CH9434_REG_OP_WRITE | CH9434_SLEEP_MOD_CFG_ADD,
                      lower_power_reg);
}
...
void CH9434GPIOFuncSet(uint8_t gpio_idx, uint8_t en, uint8_t dir, uint8_t pu,
  ...
  if (en) {
    gpio_func_reg = spi_read_write_byte(
        CH9434_REG_OP_READ | (CH9434_GPIO_FUNC_EN_0 + (gpio_idx / 8)), 0xFF);

    gpio_func_reg |= (1 << (gpio_idx % 8));

    spi_read_write_byte(CH9434_REG_OP_WRITE |
                            (CH9434_GPIO_FUNC_EN_0 + (gpio_idx / 8)),
                        gpio_func_reg);

    gpio_dir_reg = spi_read_write_byte(
        CH9434_REG_OP_READ | (CH9434_GPIO_DIR_MOD_0 + (gpio_idx / 8)), 0xFF);
    ...
    spi_read_write_byte(CH9434_REG_OP_WRITE |
                            (CH9434_GPIO_DIR_MOD_0 + (gpio_idx / 8)),
                        gpio_dir_reg);

    gpio_pu_reg = spi_read_write_byte(
        CH9434_REG_OP_READ | (CH9434_GPIO_PU_MOD_0 + (gpio_idx / 8)), 0xFF);
    ...
    spi_read_write_byte(CH9434_REG_OP_WRITE |
                            (CH9434_GPIO_PU_MOD_0 + (gpio_idx / 8)),
                        gpio_pu_reg);

    gpio_pd_reg = spi_read_write_byte(
        CH9434_REG_OP_READ | (CH9434_GPIO_PD_MOD_0 + (gpio_idx / 8)), 0xFF);
    ...
    spi_read_write_byte(CH9434_REG_OP_WRITE |
                            (CH9434_GPIO_PD_MOD_0 + (gpio_idx / 8)),
                        gpio_pd_reg);
  } else {
    gpio_func_reg = spi_read_write_byte(
        CH9434_REG_OP_READ | (CH9434_GPIO_FUNC_EN_0 + (gpio_idx / 8)), 0xFF);
...
void CH9434GPIOPinOut(uint8_t gpio_idx, uint8_t out_val) {
  ...
  spi_read_write_byte(CH9434_REG_OP_WRITE |
                          (CH9434_GPIO_PIN_VAL_0 + (gpio_idx / 8)),
                      pin_val_reg);
}
...
uint8_t CH9434GPIOPinVal(uint8_t gpio_idx) {
  ...
  pin_val_reg = spi_read_write_byte(
      CH9434_REG_OP_READ | (CH9434_GPIO_PIN_VAL_0 + (gpio_idx / 8)), 0xFF);
...
void CH9434UARTxInit(uint8_t uart_idx, uint32_t bps) {
  CH9434UARTxParaSet(uart_idx, bps, CH9434_UART_8_BITS_PER_CHAR,
                     CH9434_UART_ONE_STOP_BIT, CH9434_UART_NO_PARITY);
